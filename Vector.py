...
class Vector:
    ...
    def degree(self, centre):
        return (atan2(self.x -centre.x, self.y - centre.y)*180/pi - 90)%360

    def counterclockwise(self, centre, radians):
        radians = - radians
        return Vector((self.x - centre.x)*cos(radians) - (self.y-centre.y)*sin(radians) + centre.x,
                      (self.x - centre.x)*sin(radians) + (self.y-centre.y)*cos(radians) + centre.y)
def unit(direction):
